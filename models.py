...
class MoJingNet_e(nn.Module):
    ...
    def __init__(self, config):
        ...
        weight = torch.FloatTensor(config['weight'])
        self.embeddings = nn.Embedding.from_pretrained(weight, freeze=config['freeze_embed'])
    ...
    def encode(self, s1):
        (s1, s1_len) = s1
        emb = self.embeddings(s1)
        emb = self.encoder((emb, s1_len))
        return emb
