...
parser = argparse.ArgumentParser(description='Mojing training with cv')
# paths
...
for cv in range(10):
    ...
    while not stop_training and epoch <= params.n_epochs:
        train_acc = trainepoch(epoch, train_cv)
        eval_acc = evaluate(cv, epoch, dev_cv, False)
        epoch += 1

    # Run best model on test set.
    del mojing_net
    mojing_net = torch.load(os.path.join(params.save_dir, "%d.pkl" %(cv)))

    print('\nTEST : Epoch {0}'.format(epoch))
    evaluate(cv, 1e6, dev_cv, True)


def trainepoch(epoch, train):
    # train and dev are np.arrays
...
def evaluate(cv, epoch, dev, final_eval=False):
    mojing_net.eval()
    correct = 0.
    ...
    if epoch <= params.n_epochs:
        if eval_acc > val_acc_best:
            ...
            torch.save(mojing_net, os.path.join(params.save_dir, "%d.pkl" %(cv)))
